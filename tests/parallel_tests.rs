...
mod parallel_tests {
    ...
    fn test_tree_splitting_and_parallel_evaluation() {
        ...
        for chunk in &chunks {
            assert_eq!(chunk.len(), 2, "Expected exactly 2 leaf nodes per chunk in balanced split");
        }
        ...
        assert!((total_sum - 36.0).abs() < f64::EPSILON, "Parallel evaluation computed wrong sum");
    }
